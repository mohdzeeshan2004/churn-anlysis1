...
def load_data(uploaded_file=None):
    ...
    try:
        ...
        return pd.read_csv("European_Bank(3).csv")
...
if page == "Project Overview":
    ...
    st.markdown(
        """
        ### Background
        Customer churn is one of the biggest challenges in retail banking. When customers leave,
        banks lose revenue, customer relationship value, and future cross-selling opportunities.

        ### Problem Statement
        Banks have customer-level data, but they need clear segmentation-based insights to understand
        which customers are most likely to churn.

        ### Project Objectives
        - Analyze churn distribution
        - Compare churn by country, gender, age, balance, product usage, and activity status
        - Segment customers using K-Means clustering
        - Build a basic machine learning churn prediction model
        - Convert findings into business recommendations

        ### Tools Used
        Python, Pandas, Plotly, Scikit-learn, and Streamlit.
        """
    )
    ...
    if df is None:
        st.error("Dataset not found. Upload the CSV file from the sidebar.")
    else:
    ...
    if df is None:
        ...
        st.error("The dataset must contain an Exited column.")
    else:
        ...
        if "IsActiveMember" in filtered_df.columns:
            ...
            active_churn["Membership Status"] = active_churn["IsActiveMember"].map({
                0: "Inactive",
                1: "Active"
            })
    ...
    if df is None:
        ...
        if len(available_cols) < 3:
            ...
            st.markdown("K-Means clustering is used to create customer segments.")
            ...
            segment_df["Segment"] = kmeans.fit_predict(scaled_data).astype(str)
    ...
    if df is None:
        ...
        if len(X) < 20 or y.nunique() < 2:
            st.warning("Dataset is too small or has only one target class.")
        else:
    ...
    st.markdown(
        ...
        - High-balance customers who churn can create high-value revenue risk.
        - Inactive members are important churn-risk customers.
        - Product usage can show how deep the customer's relationship is with the bank.
        - Age groups with higher churn need targeted communication and offers.

        ### Business Recommendations
        - Create retention offers for high-balance customers.
        ...
        ### Final Outcome
        This app converts raw customer data into a dashboard, segmentation output,
        machine learning model results, and business recommendations.
        """
    )
